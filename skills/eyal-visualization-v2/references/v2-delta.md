...
- **Finding cards** on Overview after the stake chart (carousel, not actions). **Action cards** on the Recommendations tab: 2×3 CSS grid, no carousel, 1-6 cards. Type color: blue primary, purple add, mint go, yellow `#FFCB56` wait/look as type (not fill), red severe-only. 2-3 colors when enough. Labeled footer. Glow default on. See [component-recipes.md](component-recipes.md).
